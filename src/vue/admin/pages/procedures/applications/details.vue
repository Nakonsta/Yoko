<template>
  <div
    :class="[
      'application',
      { 'application--with-tabs': tabs.length && !loading },
    ]"
  >
    <application-tabs
    ...
    <div
      v-if="currentTabIndex === 0 && !loading"
    ...
    ></application-checkbox> -->
      <div
        v-if="viewType.isCreate || viewType.isDraft"
        class="application__buttons"
      >
        <button
          class="application__button"
          @click="createApplication"
        >
          Подписать и отправить
        </button>
        <button
          class="application__button application__button--second"
          @click="createApplicationDraft"
        >
          Сохранить черновик
        </button>
      </div>
      <div
        v-if="viewType.chooseWinner"
        class="application__buttons"
      >
        <button
          class="application__button"
          @click="chooseWinnerApplication"
        >
          Выбрать победителем
        </button>
        <button
...
export default {
  ...
  computed: {
    ...
    viewType() {
      return {
        ...
        isView:
          this.application.status !== 'draft' && this.applicationId !== 'new',
        chooseWinner:
          this.application.status === 'submitted'
          && this.$store.state.auth.role === 'buyer',
        winnerProcessing:
          this.application.status === 'winner_processing'
          && this.$store.state.auth.role === 'contractor',
      };
    },
    tabs() {
      const tabs = [];
      if (this.viewType.winnerProcessing) {
        if (
          this.procedure?.rebidding_info?.declared_rebidding_type
          === 'rebidding'
        ) {
  ...
  methods: {
    ...
    getApplication() {
      return this.fetchProcedureApplication(this.applicationId).then(
        ({ data }) => {
          ...
          if (data?.data?.documents?.length ?? false) {
            this.application.documents.forEach((document, i) => {
              if (this.application.documents[document.properties.group]) {
                this.application.documents[document.properties.group].push(
                  document,
                );
              } else {
                this.application.documents[document.properties.group] = [
                  document,
                ];
              }
    ...
    createApplication() {
      ...
      if (this.viewType.isCreate) {
        this.sendProcedureApplicationDraft(this.procedureId, data).then(
          ({ data }) => {
    ...
    createApplicationObject() {
      const applicationCopy = { ...this.application };
        ...
        if (value[1].length) {
          value[1].forEach((document) => {
            if (document.id) {
              ...
              applicationCopy.documents[value[0]] = applicationCopy.documents[
                value[0]
              ].filter((element) => element.id !== document.id);
            }
          });
        }
      });
    ...
    createLots() {
      ...
      this.procedure.purchase_subject.lot_amounts.forEach((amount, i) => {
        ...
        lot.amountWithVat = parseFloat(
          lot.products.reduce(
            (price, product) => price + parseFloat(product.amount_per_position),
            0,
          ),
        ).toFixed(2);
      ...
      this.procedure.purchase_subject.start_price = parseFloat(
        lots.reduce(
          (amount, lot) => amount + parseFloat(lot.amountWithVat, 10),
          0,
        ),
        10,
      )
