    ...
    >
      ...
      >
        <div class="procedure-new container-item">
          ...
          >
            <svg class="sprite-return"><use
              xmlns:xlink="http://www.w3.org/1999/xlink"
              xlink:href="\./img/sprite.svg#return"
            /></svg>
      ...
      <div class="popup__body">
        <div class="popup__content">
          <a
            href="javascript:{}"
            class="popup__close"
            @click="closeModal('#send-procedure')"
          ><svg class="sprite-close"><use
            xmlns:xlink="http://www.w3.org/1999/xlink"
            xlink:href="\./img/sprite.svg#close"
          /></svg></a>
          <div class="popup__title">
            Процедура направлена на согласование оператором
          </div>
...
export default {
  ...
  methods: {
    ...
    parseData(data) {
      ...
      const tender_trading_format = this.fieldsData.tenderTradingFormat.find((x) => x.id === data.tender_trading_format);
      const alternative_applications = this.fieldsData.alternativeApplications.find((x) => x.id === this.parseBoolToNumber(data.alternative_applications));
      const analog_products = this.fieldsData.goodsAnalogs.find((x) => x.id === this.parseBoolToNumber(data.purchase_subject.products_analogues));
      const tender_available = this.fieldsData.tenderAvailable.find((x) => x.id === this.parseBoolToNumber(data.tender_available));
      const stages_of_the_procurement_procedure = this.fieldsData.stagesProcedure.find((x) => x.id === this.parseBoolToNumber(data.many_stages_of_procurement));
      ...
      const calculate_the_amount_of_collateral = data.guarantee && this.fieldsData.amountOfCollateral.find((x) => x.id === data.guarantee.contract_collateral.calc_amount);
      const blocking_period_days = data.guarantee && this.fieldsData.blockingPeriodDays.find((x) => x.id === parseInt(data.guarantee.contract_collateral.blocking_period));
      const calculate_the_amount_of_collateralA = data.guarantee && this.fieldsData.amountOfCollateral.find((x) => x.id === data.guarantee.application_collateral.calc_amount);
      const blocking_period_daysA = data.guarantee && this.fieldsData.blockingPeriodDays.find((x) => x.id === parseInt(data.guarantee.application_collateral.blocking_period));
      this.selectedData = {
        ...this.selectedData,
        currency,
        count_lots,
    ...
    countTotalPrice(index) {
      ...
      const price = this.selectedData.positions[index].price_for_one ? this.selectedData.positions[index].price_for_one.toString().replace(/[^0-9.]/g, '') : 0;
    ...
    countCollateralAmount(type) {
      this.$nextTick(() => {
        this.selectedData[type].collateral_amount_percents = this.selectedData[type].percentage_of_the_starting_price
    ...
    sendNewProcedureData(toPublish, toModeration = false) {
      const formData = {
        ...
        purchase_subject: {
          ...
          start_price: this.procedureIdData.baseCount.toString().replace(/[^0-9.]/g, ''),
          products: [],
          lot_amounts: [],
        },
        purchase_term: {
      ...
      if (this.selectedData.positions[0].type && this.selectedData.positions.length) {
        ...
        tempPosition.map((item, index) => {
          ...
          formData.purchase_subject.products.forEach((item) => {
            ...
            item.price_for_one = item.price_for_one.toString().replace(/[^0-9.]/g, '');
          });
      ...
      if (this.selectedData.application_security_of_the_contract) {
        ...
        if (this.selectedData.security.calculate_the_amount_of_collateral.id === 'percent') {
          formData.guarantee.contract_collateral = {
            ...formData.guarantee.contract_collateral,
            amount: this.selectedData.security.collateral_amount_percents.toString().replace(/[^0-9.]/g, ''),
        ...
        if (this.selectedData.security.calculate_the_amount_of_collateral.id === 'monetary_expression') {
          formData.guarantee.contract_collateral = {
            ...formData.guarantee.contract_collateral,
            amount: this.selectedData.security.collateral_amount.toString().replace(/[^0-9.]/g, ''),
          };
        }
      }
      if (this.selectedData.application_security_required) {
        ...
        if (this.selectedData.request.calculate_the_amount_of_collateral.id === 'percent') {
          formData.guarantee.application_collateral = {
            ...formData.guarantee.application_collateral,
            amount: this.selectedData.request.collateral_amount_percents.toString().replace(/[^0-9.]/g, ''),
        ...
        if (this.selectedData.request.calculate_the_amount_of_collateral.id === 'monetary_expression') {
          formData.guarantee.application_collateral = {
            ...formData.guarantee.application_collateral,
            amount: this.selectedData.request.collateral_amount.toString().replace(/[^0-9.]/g, ''),
          };
        }
      }
