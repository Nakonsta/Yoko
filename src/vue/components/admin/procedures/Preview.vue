<template>
  <div>
    <div class="tabs tabs--line js-tabs">
      <ul class="mt3">
        <li class="active">
          <a href="#description">Извещения</a>
        </li>
        <li class="">
          <a href="#person">Организатор</a>
        </li>
      </ul>
    </div>
    <div
      id="description"
      class="tabs__content company__content"
      style="display: block;"
    >
      <div class="container-item">
        ...
        <div class="preview-block">
          <previews
            ...
            :value="convertedData && convertedData.publication_date"
          />
          <previews
            v-if="
          ...
          <previews
            ...
            :value="data.alternative_applications.name"
          />
          <previews
            label="Допускается ли толеранс"
            :value="convertedData.tender_tolerance"
          />
          <previews
            v-if="
              calculatedData.procedureType === 'Query' ||
        ...
        <div class="preview-block">
          ...
          <previews
            ...
            :value="convertedData.baseCount"
          />
        </div>
        <div
          v-for="index in data.count_lots.id || 1"
          :key="index"
        >
          <ul
            v-if="data.count_lots.id !== 0"
            class="mt2"
          >
            <li><b>Лот {{ index }}</b></li>
          ...
          <table class="table table--adaptive">
            ...
            <tbody>
              <tr
                ...
                v-if="item.addLot ? item.addLot.id === index : true"
              >
                <td data-label="Наименование позиции">
                ...
                <td data-label="Единица">
                  {{ item.measures && item.measures.name }}
                </td>
                <td data-label="Валюта">
                  {{ data.currency && data.currency.name }}
                </td>
                <td data-label="Стоимость за единицу">
                  {{ item.price_for_one }}
                </td>
                <td data-label="НДС">
                  {{ item.vats && item.vats.name }}
                </td>
                <td data-label="Сумма за позицию">
                  {{ item.total_price }}
                </td>
              </tr>
            </tbody>
        ...
        <div
          v-if="procedureIdData.procedureType === 'Commercial' && !fieldsData.hideBlock.payment_info"
          class="preview-block"
        >
          <previews
            label="Предоплата"
            :value="convertedData.application_prepayment"
          />
          <previews
            label="Аккредитив"
            :value="convertedData.application_letter_of_credit"
          />
          <previews
            label="Информация об оплате"
            :value="data.application_payment_info"
          />
          <previews
            label="Условия поставки"
            :value="data.application_delivery_conditions"
          />
          <previews
            label="Место поставки"
            :value="data.applications_delivery_address && data.applications_delivery_address.name"
          />
        </div>
        ...
        <h3 v-if="data.application_security_of_the_contract || data.application_security_required">
        ...
        >
          <previews
            ...
            :value="data.security.calculate_the_amount_of_collateral && data.security.calculate_the_amount_of_collateral.name"
          />
          <previews
            label="Процент от начальной цены"
            :value="data.security.percentage_of_the_starting_price"
          />
          <previews
            label="Сумма обеспечения"
            :value="data.security.collateral_amount_percents"
          />
          <previews
            label="Сумма обеспечения"
            :value="data.security.collateral_amount"
          />
          <previews
            label="Срок блокировки, дней"
            :value="data.security.blocking_period_days && data.security.blocking_period_days.name"
          />
        </div>
        <div
          v-if="procedureIdData.procedureType === 'Commercial'
        ...
        >
          <previews
            label="Считать размер обеспечения в"
            :value="data.request.calculate_the_amount_of_collateral && data.request.calculate_the_amount_of_collateral.name"
          />
          <previews
            label="Процент от начальной цены"
            :value="data.request.percentage_of_the_starting_price"
          />
          <previews
            label="Сумма обеспечения"
            :value="data.request.collateral_amount_percents"
          />
          <previews
            label="Сумма обеспечения"
            :value="data.request.collateral_amount"
          />
          <previews
            label="Срок блокировки, дней"
            :value="data.request.blocking_period_days && data.request.blocking_period_days.name"
          />
        </div>
        ...
        <div
          v-if="procedureIdData.procedureType === 'Commercial' && !fieldsData.hideBlock.additional_info"
          class="preview-block"
        >
          <previews
            label="Дополнительная информация"
            :value="data.addition_information"
          />
        </div>
        <h3 v-if="procedureIdData.procedureType === 'Commercial' && !fieldsData.hideBlock.documentation">
          Конкурсная документация
        </h3>
        <div
          v-if="procedureIdData.procedureType === 'Commercial' && !fieldsData.hideBlock.documentation"
          class="preview-block"
        >
          <div class="preview-block__item">
            <div class="preview-block__key">
              Загруженные документы
            </div>
            <div class="preview-block__value">
              <div
                v-for="item in data.file"
                v-if="data.file.length"
                class="link"
              >
        ...
        <div class="preview-block">
          <previews
            label="ФИО"
            :value="data.contact_full_name.name"
          />
          <previews
            label="Телефон"
      ...
      <div class="container-item">
        ...
        <div class="preview-block">
          <previews
            ...
            :value="convertedData.companyInfo.directorFio"
          />
          <previews
          ...
          <previews
            ...
            :value="data.contact_full_name.phone"
          />
          <previews
            label="E-mail"
            :value="data.contact_full_name.email"
          />
        </div>
        ...
        <div class="preview-block">
          ...
          <previews
            ...
            :value="data.companyName.shortName"
          />
          <previews
            label="ИНН"
            :value="data.companyName.inn"
          />
          <previews
            label="ОГРН/ОГРНИП"
            :value="data.companyName.ogrn"
          />
          <previews
            label="КПП"
            :value="data.companyName.kpp"
          />
          <previews
            label="Страна"
          ...
          <previews
            label="Адрес"
            :value="data.companyName.address"
          />
          <previews
            label="Почтовый индекс"
            :value="data.companyName.post"
          />
        </div>
      </div>
    </div>
