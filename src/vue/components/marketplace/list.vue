  ...
  <div class="procedures items">
    ...
    <div class="procedures__items items">
      <div
        ...
        class="procedures__item procedures__item--empty item item--empty"
      >
        По вашему запросу ничего не найдено
      </div>
      <div
        v-if="!items.length && loading"
        class="procedures__item procedures__item--loading item item--loading"
      >
        <div class="preloader">
      ...
      <div
        ...
        v-if="isFirstLoad && items.length"
        :key="item.id"
        class="procedures__item item"
      >
        <div class="procedures__item-head">
          <div class="procedures__item-flex">
            <div class="procedures__item-title">
              №{{ item.id }}
            </div>
            <div class="procedures__item-type">
              {{ getTradingFormat(item.tender_trading_format) }}:
              {{ getTradingType(item.tender_trading_type) }}
            </div>
          </div>
          <dl>
            <dt>Компания</dt>
            <dd>
              <a
                v-if="item.company"
                :href="'/compregister/'+item.company.id"
              >{{ item.company.name }}</a>
            </dd><!-- todo линк на компанию -->
          </dl>
        ...
        <div class="procedures__item-body">
          <div class="procedures__item-flex">
            <div class="procedures__item-status">
            ...
            <div class="procedures__item-btns">
              <a
                href="javascript:{}"
                title="Распечатать"
                @click="printPage(`/platform/${item.id}`)"
              ><svg class="sprite-print"><use
                xmlns:xlink="http://www.w3.org/1999/xlink"
                xlink:href="\./img/sprite.svg#print"
              /></svg></a>
              <a
                :href="`/platform/${item.id}#documents`"
                title="Приложенные файлы"
              ><svg class="sprite-paperclip"><use
                xmlns:xlink="http://www.w3.org/1999/xlink"
              ...
              <a
                ...
                @click="updateItemMark(item, 'hidden', itemMarkExist(item, 'hidden') ? 'Процедура показана' : 'Процедура скрыта')"
              >
                <svg class="sprite-hide"><use
                  xmlns:xlink="http://www.w3.org/1999/xlink"
                  xlink:href="\./img/sprite.svg#hide"
                /></svg>
              </a>
              <a
                ...
                @click="updateItemMark(item, 'favorite', itemMarkExist(item, 'favorite') ? 'Процедура удалена из избранного' : 'Процедура добавлена в избранное')"
              >
                <svg class="sprite-favorite"><use
                  xmlns:xlink="http://www.w3.org/1999/xlink"
                  xlink:href="\./img/sprite.svg#favorite"
                /></svg>
              </a>
            </div>
          </div>
          ...
          <div class="procedures__item-dates">
            <div
              v-if="item.publication_date"
              class="procedures__item-date"
            >
              <span>Дата размещения:</span>
              {{ formatDateNoTime(item.publication_date) }}
            </div>
            <div
              v-if="item.purchase_term && item.purchase_term.procedure_date_to"
              class="procedures__item-date"
            >
        ...
        <div class="procedures__item-foot">
          <div class="procedures__item-flex">
            <a
              v-if="!expandAll"
              href="javascript:{}"
              class="procedures__item-toggle"
              @click="toggleItem(index)"
            >
              <span v-html="item.isExpanded ? 'Скрыть' : 'Показать позиции'" />
              <svg
          ...
          <div
            v-if="item.isExpanded || expandAll"
            class="procedures__item-products"
          >
            <table>
              <colgroup>
                <col>
...
export default {
  ...
  methods: {
    ...
    openPopup(evt) {
      ...
      const popupContainer = document.querySelector('#default .popup__content-container');
