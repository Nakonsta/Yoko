export default {
  ...
  methods: {
    ...
    cancelCompanyRequest() {
      this.CancelTokens.companyCancelToken.cancel('Предыдущий запрос отменен');
      this.CancelTokens.companyCancelToken = axios.CancelToken.source();
    },
    cancelMarketplaceProceduresRequest() {
      this.CancelTokens.proceduresCancelToken.cancel('Предыдущий запрос отменен');
    ...
    sendSettingsData(string, data) {
      if (!this.CancelTokens.catalogProceduresCancelToken) {
        ...
        this.CancelTokens.catalogProceduresCancelToken.cancel('Предыдущий запрос отменен запрос');
        this.CancelTokens.catalogProceduresCancelToken = axios.CancelToken.source();
      }

      return axios.post(`${process.env.API_URL_AUTH_SERVICE}/user/settings/${string}`, data, {
        cancelToken: this.CancelTokens.catalogProceduresCancelToken.token,
      });
    },
    fetchCatalog(filter = null, page = 1) {
      return axios.post(
        `${process.env.API_URL_CONTENT_SERVICE}/api/catalog/`,
        {
          ...filter,
          page,
        },
    ...
    fetchTotalCatalog(group, filter) {
      ...
      return axios.post(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/total/`, body, {
        cancelToken: this.CancelTokens.catalogCancelToken.token,
      });
    },
    searchProceduresOKPD2(string) {
      return axios.get(`${process.env.API_URL_CONTENT_SERVICE}/api/digests/okpd/search/`, { params: { q: string } });
    },
    ...
    getDate(data) {
      return axios.get(`${process.env.API_URL_CONTENT_SERVICE}/api/settings/tender/`, data);
    },
    forgotPassChange(data) {
      return axios.post(`${process.env.API_URL_AUTH_SERVICE}/user/restore_password`, data);
    },
    fetchSupportInfo() {
    ...
    fetchCompany(data) {
      return axios.post(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/company`, data, {
    ...
    cancelCatalogSearch() {
      this.CancelTokens.searchCancelToken.cancel('Предыдущий запрос отменен');
      this.CancelTokens.searchCancelToken = axios.CancelToken.source();
    },
    fetchCatalogMark(string) {
      return axios.get(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/search/mark/`, {
        params: {
          q: string,
        },
    ...
    cancelCatalogMarkSearch() {
      this.CancelTokens.searchMarkCancelToken.cancel('Предыдущий запрос отменен');
      this.CancelTokens.searchMarkCancelToken = axios.CancelToken.source();
    },
    fetchCatalogMarksize(string) {
      return axios.get(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/search/marksize/`, {
        params: {
          q: string,
        },
    ...
    fetchCatalogMarksizeOKPD(string, okpd) {
      return axios.get(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/search/marksize_with_okpd/`, {
        params: {
          q: string,
          okpd_2_id: okpd,
        },
    ...
    sendProcedure(data, id) {
      const setId = id ? `/${id}` : '';
      return axios.post(`${process.env.API_URL_TENDER_SERVICE}/api/procedure${setId}`, data);
    },
    fetchDaData(query) {
      return axios.post(
        process.env.DA_URL,
    ...
    filterMarksizeQuantity(id, data) {
      return axios.post(`${process.env.API_URL_CONTENT_SERVICE}/api/catalog/marksize/${id}/quantity/`, data, {
        cancelToken: this.CancelTokens.companyCancelToken.token,
      });
    },
